...
public class PartPlacement {
    ...
    public static EnumActionResult place(final ItemStack held, final BlockPos pos, EnumFacing side,
            final EntityPlayer player, final EnumHand hand, final World world, PlaceType pass, final int depth) {
        if (depth > 3) {
                ...
                if (!world.isRemote) {
                    ...
                    if (mop != null) {
                        ...
                        final SelectedPart sp = selectPart(player, host,
                                mop.hitVec.addVector(-mop.getBlockPos().getX(), -mop.getBlockPos().getY(),
        ...
        if (held.isEmpty()) {
            ...
            if (host != null && player.isSneaking() && block != null) {
                ...
                if (mop != null) {
                    mop.hitVec = mop.hitVec.addVector(-mop.getBlockPos().getX(), -mop.getBlockPos().getY(),
                            -mop.getBlockPos().getZ());
        ...
        if (event instanceof PlayerInteractEvent.RightClickEmpty && event.getEntityPlayer().world.isRemote) {
            ...
            if (mop != null && mop.hitVec.distanceTo(vec3) < d0) {
                ...
                if (event.getEntityPlayer().isSneaking() && !held.isEmpty() && supportedItem) {
                    NetworkHandler.instance()
                            .sendToServer(new PacketClick(event.getPos(), event.getFace(), 0, 0, 0, event.getHand()));
                }
            }
            ...
            final ItemStack held = event.getEntityPlayer().getHeldItem(event.getHand());
            if (place(held, event.getPos(), event.getFace(), event.getEntityPlayer(), event.getHand(),
