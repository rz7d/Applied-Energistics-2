...
public class Platform {
    ...
    public static EnumFacing crossProduct(final EnumFacing forward, final EnumFacing up) {
        final int west_x = forward.getFrontOffsetY() * up.getFrontOffsetZ()
                - forward.getFrontOffsetZ() * up.getFrontOffsetY();
        final int west_y = forward.getFrontOffsetZ() * up.getFrontOffsetX()
                - forward.getFrontOffsetX() * up.getFrontOffsetZ();
        final int west_z = forward.getFrontOffsetX() * up.getFrontOffsetY()
                - forward.getFrontOffsetY() * up.getFrontOffsetX();
                ...
                if (!i.isEmpty()) {
                    if (i.getCount() > 0) {
                        ...
                        final EntityItem ei = new EntityItem(w, 0.5 + offset_x + pos.getX(),
                                0.5 + offset_y + pos.getY(), 0.2 + offset_z + pos.getZ(), i
                                        .copy());
                        w.spawnEntity(ei);
                    }
                }
    ...
    public static RayTraceResult rayTrace(final EntityPlayer p, final boolean hitBlocks, final boolean hitEntities) {
        ...
        final AxisAlignedBB bb = new AxisAlignedBB(Math.min(vec3.x, vec31.x), Math.min(vec3.y, vec31.y),
                Math.min(vec3.z,
                        vec31.z),
                Math.max(vec3.x, vec31.x), Math.max(vec3.y, vec31.y), Math.max(vec3.z, vec31.z)).grow(
                        16, 16, 16);
    ...
    public static ItemStack extractItemsByRecipe(final IEnergySource energySrc, final IActionSource mySrc,
        ...
        if (energySrc.extractAEPower(1, Actionable.SIMULATE, PowerMultiplier.CONFIG) > 0.9) {
            ...
            final boolean checkFuzzy = ae_req.getOre().isPresent()
                    || providedTemplate.getItemDamage() == OreDictionary.WILDCARD_VALUE || providedTemplate
                            .hasTagCompound()
            ...
            if (items != null && checkFuzzy) {
                for (final IAEItemStack x : items) {
                    ...
                    if ((Platform.itemComparisons().isEqualItemType(providedTemplate, sh) || ae_req.sameOre(x))
                            && !ItemStack.areItemsEqual(sh,
                                    output)) { // Platform.isSameItemType( sh, providedTemplate )
    ...
    // )
    // {
    // final EntityPlayer p = AEApi.instance().registries().players().findPlayer(
    // playerID );
    // if( p != null )
    // {
