...
public interface IItemRendering {
    ...
    /**
     * Registers a custom item mesh definition that will be used to dynamically
     * determine the item model to be used for rendering by inspecting the item
     * stack (i.e. for NBT data). Please
     */
    @SideOnly(Side.CLIENT)
    IItemRendering meshDefinition(ItemMeshDefinition meshDefinition);

    /**
     * Registers an item model for meta=0, see
     * {@link #model(int, ModelResourceLocation)}.
     */
    @SideOnly(Side.CLIENT)
     ...
     *
     * Models registered via {@link #model(int, ModelResourceLocation)} are
