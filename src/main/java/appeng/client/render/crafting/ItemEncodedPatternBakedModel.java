...
/**
 * This special model handles switching between rendering the crafting output of
 * an encoded pattern (when shift is being held), and showing the encoded
 * pattern itself. Matters are further complicated by only wanting to show the
 * crafting output when the pattern is being rendered in the GUI, and not
 * anywhere else.
 */
class ItemEncodedPatternBakedModel implements IBakedModel {
    ...
    private class ShiftHoldingModelWrapper implements IBakedModel {
        ...
        public Pair<? extends IBakedModel, Matrix4f> handlePerspective(
            ...
            // Now retroactively handle the isGui3d call, for which we always return false
            // below
            ...
            return PerspectiveMapWrapper.handlePerspective(this, ItemEncodedPatternBakedModel.this.transforms,
                    cameraTransformType);
        }

        @Override
        public List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, long rand) {
            // This may be called for items on the ground, in which case we will always fall
            // back to the pattern
        ...
        @Override
        public boolean isGui3d() {
            // NOTE: Sadly, Forge will let Minecraft call this method before handling the
    ...
    private class CustomOverrideList extends ItemOverrideList {
        ...
        public IBakedModel handleItemState(IBakedModel originalModel, ItemStack stack, World world,
            ...
            if (shiftHeld) {
                ...
                if (!output.isEmpty()) {
                    IBakedModel realModel = Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
                            .getItemModel(output);
