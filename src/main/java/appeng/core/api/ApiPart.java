...
public class ApiPart implements IPartHelper {
    ...
    /**
     * Conceptually this method will build a new class hierarchy that is rooted at
     * the given base class, and includes a chain of all registered layers.
     * <p/>
     * To accomplish this, it takes the first registered layer, replaces it's
     * inheritance from LayerBase with an inheritance from the given baseClass, and
     * uses the resulting class as the parent class for the next registered layer,
     * for which it repeats this process. This process is then repeated until a
     * class hierarchy of all layers is formed. While janking out the inheritance
     * from LayerBase, it'll make also sure that calls to that classes method will
     * instead be forwarded to the superclass that was inserted as part of the
     * described process.
     * <p/>
     * Example: If layers A and B are registered, and TileCableBus is passed in as
     * the baseClass, a synthetic class A_B_TileCableBus should be returned, which
     * has A_B_TileCableBus -extends-> B_TileCableBus -extends-> TileCableBus as
     * it's class hierarchy, where A_B_TileCableBus has been generated from A, and
     * B_TileCableBus has been generated from B.
     */
    ...
    private Class<? extends AEBaseTile> generateCombinedClass(CacheKey cacheKey) {
        ...
        // Get the list of interfaces that still need to be implemented beyond the
        // current one
        ...
        try {
            // This is the particular interface that this layer was registered for. Loading
            // the class may fail if i.e.
    ...
    private Class loadClass(final String name, byte[] b) {
        ...
        try {
            ...
            final Method defineClassMethod = root.getDeclaredMethod("defineClass", String.class, byte[].class,
                    int.class, int.class);
            final Method runTransformersMethod = cls.getDeclaredMethod("runTransformers", String.class, String.class,
                    byte[].class);
