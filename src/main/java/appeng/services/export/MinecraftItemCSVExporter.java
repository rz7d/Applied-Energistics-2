...
/**
 * handles the exporting including processing, transformation and persisting the
 * information
 *
 * @author thatsIch
...
final class MinecraftItemCSVExporter implements Exporter {
    ...
    /**
     * @param exportDirectory directory of the resulting export file. Non-null
     *                        required.
     * @param itemRegistry    the registry with minecraft items. Needs to be
     *                        populated at that time, thus the exporting can only
     *                        happen in init (pre-init is the phase when all items
     *                        are determined)
     * @param mode            mode in which the export should be operated. Resulting
     *                        CSV will change depending on this.
     */
    ...
    private static final class ItemRowExtractFunction implements Function<Item, String> {
         ...
         * @param mode         extracts more or less information from item depending on
         *                     mode
         */
