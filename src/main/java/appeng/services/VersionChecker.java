...
/**
 * Tries to connect to GitHub to retrieve the most current build. After
 * comparison with the local version, several path can be chosen.
 *
 * If the local version is invalid, somebody might have build that version
 * themselves or it is run in a developer environment, then nothing needs to be
 * done.
 *
 * If GitHub can not be reached, then either is GitHub down or the connection to
 * GitHub disturbed, then nothing needs to be done, since no comparison can be
 * reached
 *
 * If the version was just recently checked, then no need to poll again. Nobody
 * wants to bother to update several times a day.
 *
 * Config enables to fine-tune when a version is considered newer
 *
 * If the local version is newer or equal to the GitHub version, then no update
 * needs to be posted
 *
 * Only after all that cases, if the external version is higher than the local,
 * use Version Checker Mod and post several information needed for it to update
 * the mod.
 */
public final class VersionChecker implements Runnable {
    ...
    public void run() {
        try {
            ...
            // Log any unhandled exception to prevent the JVM from reporting them as
            // unhandled.
     ...
     * @param lastAfterInterval last version check including the interval defined in
     *                          the config
     */
    ...
    /**
     * Checks if the retrieved version is newer as the current mod version. Will
     * notify player if config is enabled.
     *
    ...
    /**
     * Checks if the version checker mod is installed and handles it depending on
     * that information
     *
